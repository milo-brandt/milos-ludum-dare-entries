...
class Vector{
    ...
    constexpr Vector(CallbackConstructImpl_t, Callback&& callback, std::index_sequence<indices...>):data{callback(indices)...}{}
public:
    constexpr Vector():data{}{}
    template<class... Args, class = std::enable_if_t<sizeof...(Args) == d && (std::is_convertible_v<Args, T> && ...)> >
    constexpr Vector(Args&&... args):data{(T)std::forward<Args>(args)...}{}
    template<class Callback>
    constexpr Vector(CallbackConstruct_t, Callback&& callback):Vector(CallbackConstructImpl, std::forward<Callback>(callback), std::make_index_sequence<d>{}){}
    template<class T2, class = std::enable_if_t<std::is_convertible_v<T2, T> > >
    constexpr Vector(Vector<T2, d> const& other):Vector(CallbackConstruct, [&other](std::size_t i) -> T{ return other[i]; }){}
    ...
    Vector parallelComponentOf(Vector const& other){
        return *this * (dot(other) / magnitudeSq());
    }
    Vector perpendicularComponentOf(Vector const& other){
        return other - parallelComponentOf(other);
    }
