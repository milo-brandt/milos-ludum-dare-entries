...
public class WorldSumo extends PApplet {
...
public void startGame(){
  ...
  x = new Player(new Vector2(0,0),planets.get(0),10,4.5f*scaleF);
  score = 0;
}
public boolean drawButton(float x,float y,float halfw,float halfh){
...
float boxSize = 30;
public void stars(){ 
 int rseed = PApplet.parseInt(random(0,1000000));
 ...
 for(int i = 0;i <= ceil(cullDist*2/boxSize);++i){
   for(int j = 0;j <= ceil(cullDist*2/boxSize);++j){
     ...
     randomSeed(PApplet.parseInt(corn.x*999 + corn.y*9999));
     if(random(1) > 0.7f){
     fill(random(1)*255);
...
public void drawArrow(){ //Dear DRY: Sorry
...
public boolean isHuge(){
  ...
  if(score < 3500) return random(1) > 0.975f;
  if(score < 7000) return random(1) > 0.95f;
  return random(1) > 0.9f;
}
...
public void spawnFree(){
  ...
  float speed = (isSpeedy()?scaleF*random(1,2):scaleF*2.5f)*getMinSpeed();
  float timeAway = 10;
  ...
  planets.add(new Planet(radi,Vector2.sum(pos,target),0,random(-0.05f,0.05f),Vector2.sum(x.home.veloc,Vector2.angleRadius(lastFree,-speed)),timeAway*frames+totalTime));
}
public void drawSideBox(Vector2 pos,float rad,float h){ //Yeah, this function is mostly polish.
  if(abs(abs(pos.x) - width/2) < 1.0f){
    boolean neg = pos.x < 0;
...
public void physics(float dt){
  ...
  for(int i = 0;i < planets.size();++i){
    ...
    if(b.health <= 0 || b.pos.magnitude() > maxDist + 1.5f*cullDist){
      if(b == x.on){
        x.on = null;
...
public void draw() {
  ...
  drawDistometer();
    if(!x.alive){
    deathProgress = min(1,deathProgress + 1/120.0f);
  ...
  text("Score: " + PApplet.parseInt(score),width/2,30*(1-deathProgress)+height/2*deathProgress);
    if(transitionIn != 0){
    transitionIn = max(0,transitionIn - 1/120.0f);
  ...
  if(mode == 0){
    drawTitle();
           if(transitionIn != 0){
      transitionIn = max(0,transitionIn - 1/60.0f);
      fill(0,0,0,transitionIn*255);
      rect(0,0,width,height);
           fill(255);

    }    if(deathProgress != 0){
      deathProgress = min(1,deathProgress + 1/60.0f);
      fill(0,0,0,deathProgress*255);
      rect(0,0,width,height);
      fill(255);
    }
  ...
  if(mode == 2){
    ...
    textFont(r.score,60);
    text("Score: " + PApplet.parseInt(score),width/2,height/2);
       if(deathProgress != 0){
      deathProgress = min(1,deathProgress + 1/60.0f);
      fill(0,0,0,deathProgress*255);
      rect(0,0,width,height);
...
class Bounds{
  ...
  public Vector2 transform(Vector2 world){
  ...
  public Vector2 transformSpeed(Vector2 r){
...
class Planet{
  ...
  public Planet(float maxRadius,Vector2 pos,float rot,float rotVeloc,Vector2 veloc,float expectedImpact){
    ...
    this.health = 200;
    this.imgNum = PApplet.parseInt(random(0,4));
  }
...
class Player{
  ...
  public void setCamera(){
    ...
    camDown = new Vector2(camDown.x * 0.9f + home.pos.x * 0.1f,camDown.y * 0.9f + home.pos.y * 0.1f);
    ...
    image(jumping?r.playerAnimation[1]:running?r.playerAnimation[PApplet.parseInt(totalTime/7)%2]:r.still,0,0,20,20);
    popMatrix();
    beginCamera();
...
class Resources{
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "WorldSumo" });
  }
}
